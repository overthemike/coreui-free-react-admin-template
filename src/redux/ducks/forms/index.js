...
function submitForm (inquiry, wallet_updated, notes, dispatch) {
  return new Promise((resolve, reject) => {
    axios
      .post("/customer-requests/card-inquiry/", { inquiry, wallet_updated, notes, dispatch }, {headers: {token: window.localStorage.getItem('token')}})
      .then(resp => {
        dispatch({
          type: REQUEST_CARD,
...
export function useForms() {
  ...
  const wallet_updated = useSelector(appState => appState.formState.wallet_updated)
  const notes = useSelector(appState => appState.formState.notes)
  const requestCard = (inquiry, wallet_updated, notes) => submitForm(inquiry, wallet_updated, notes, dispatch)
